Cybersecurity Learning Path

Hi, this is my first GitHub repository.
<br>
Here I will share my cybersecurity learning journey, notes, tools, practice labs, and web security projects.
<br>
##About Me
Author: Lucky Naruka

What This Repository Contains
--Cybersecurity learning notes
--Hands-on practice and lab work
--Tools and commands I learn
--Web security concepts and writeups
--Beginner cybersecurity projects
##Topics Covered
--Kali Linux
--Linux Commands using OverTheWire
--Networking Basics
--Web Application Security
--Vulnerabilities using PortSwigger Web Security Academy
##Security Testing Tools:
-Burp Suite
##Current Focus
Server-side vulnerabilities
Practical web security testing
## Goals
--Improve practical cybersecurity skills
--Learn web application security deeply
--Build cybersecurity projects
--Create security writeups and notes
--Practice ethical hacking in lab environments
##Platforms Used
--OverTheWire
--PortSwigger Web Security Academy
##Disclaimer

This repository is created for educational and ethical learning purposes only.
<br>
All testing and practice are performed in legal lab environments such as practice labs and intentionally vulnerable applications.
<br>
